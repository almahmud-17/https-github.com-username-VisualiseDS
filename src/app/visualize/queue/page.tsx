        ...
        <main className="min-h-screen pt-24 pb-12 flex flex-col items-center">
            ...
            <div className="container max-w-7xl flex-1 flex flex-col gap-8 px-6">
                <div className="flex flex-col gap-2">
                    <h1 className="text-4xl font-bold tracking-tight text-foreground text-center md:text-left">
                        Queue Visualizer
                    </h1>
                ...
                <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-4 flex flex-col gap-6 order-2 lg:order-1">
                        ...
                        <div className="grid grid-cols-2 gap-3 px-1">
                            <PremiumButton onClick={() => handleEnqueue(Math.floor(Math.random() * 99) + 1)} variant="gradient" className="col-span-2 h-12 text-[10px] font-black uppercase tracking-widest">
                                <RotateCcw size={14} className="mr-2" /> Enqueue Random Value
                            </PremiumButton>
                            <PremiumButton onClick={handleFront} variant="secondary" disabled={queue.length === 0}>
                                <Eye size={16} /> Front
                            </PremiumButton>
                            <PremiumButton onClick={handleClear} variant="danger" disabled={queue.length === 0}>
                                <RotateCcw size={16} /> Clear
                            </PremiumButton>
                        </div>

                        <div className="h-[450px]">
                            <CodePanel code={QUEUE_CODE} currentLine={currentLine} />
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-8 glass-card min-h-[550px] flex flex-col items-center justify-center p-8 order-1 lg:order-2 relative overflow-hidden">
                            ...
                            <AnimatePresence mode="popLayout">
                                {queue.map((val, idx) => (
                                    <motion.div
                                        ...
                                        className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center rounded-2xl font-bold shadow-xl border border-white/10 relative group"
                                    >
                                        <span className="text-xl">{val}</span>

                                        {idx === 0 && (
                                            <div className="absolute -top-10 flex flex-col items-center gap-1">
                                                <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-tighter">FRONT</span>
                                        ...
                                        {idx === queue.length - 1 && queue.length > 1 && (
                                            <div className="absolute -bottom-10 flex flex-col items-center gap-1">
                                                ...
                                                <span className="text-[10px] font-bold text-teal-400 uppercase tracking-tighter">REAR</span>
                                            </div>
                                        )}
                                    </motion.div>
                                ))}
                        ...
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-8 opacity-20">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-bold uppercase">Enqueue End</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full bg-red-500" />
                                <span className="text-[10px] font-bold uppercase">Dequeue End</span>
                            </div>
                        </div>
                    </div>
                </div>
